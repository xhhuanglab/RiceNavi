...
if(@ARGV < 2){
  die "Usage: perl RiceNavi-Sim_run_scripts.pl <RiceNavi-Sim.cfg> <SelectedGene.loci>";	
}else{
  ($sim_cfg, $selected_genelist) = @ARGV;	
}
...
mkdir "parallel_cmd_dir";
mkdir "simulation_dir";

`cp $selected_genelist ./parallel_cmd_dir`;
`cp SimUtils/* ./parallel_cmd_dir`;
`cp Scripts/* ./parallel_cmd_dir`;
`cp Scripts/stat_likelihood.pl ./simulation_dir`;
...
for my $sim_num (1..$simulation_times){
   ...
   if($sim_num % $break_simulation == 1){
     ...
     open BASH, ">parallel_cmd_dir/simulation_group.$count.sh";     	
   }
     my $sim_group_sub = $sim_num % ($simulation_times/$cut_num);
     my $sim_outdir = '../simulation_dir/sim_group'.$count.'.'.$sim_group_sub;
